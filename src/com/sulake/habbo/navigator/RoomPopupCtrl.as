package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2725:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2725 = new RoomThumbnailRenderer(param1);
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_198 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_198.visible = true;
         Util.hideChildren(this.var_198);
         this.refreshRoomName(this.var_198,this.var_130);
         if(this.var_130.ownerName != "" && this.var_130.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_198,!!this.var_130.event ? this.getEventCtg(this.var_130.categoryId) : this.getRoomCtg(this.var_130.categoryId));
         this.refreshRoomDesc(this.var_198,this.var_130);
         this.refreshExtraCont();
         this.refreshInfo(this.var_198,"trading_allowed",this.var_130.allowTrading);
         this.refreshInfo(this.var_198,"doormode_doorbell",this.var_130.doorMode == 1);
         this.refreshInfo(this.var_198,"doormode_password",this.var_130.doorMode == 2);
         Util.moveChildrenToColumn(this.var_198,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_198.height = Util.getLowestPoint(this.var_198);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_198,this.var_130.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_198.findChildByName("roomowner"));
         if(this.var_130.ownerName != "" && this.var_130.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_198.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_986.refreshTags(_loc1_,this.var_130.tags);
         ...
         this.var_2725.refreshThumbnail(_loc1_,this.var_130.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_208,Tabs.const_242,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
