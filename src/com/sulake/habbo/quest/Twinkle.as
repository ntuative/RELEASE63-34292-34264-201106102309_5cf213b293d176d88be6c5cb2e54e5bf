package com.sulake.habbo.quest
{
   ...
   public class Twinkle implements AnimationObject, IDisposable
   {
      ...
      private static const const_1010:Array = [1,2,3,4,5,6,5,4,3,2,1];
      
      private static const const_1389:int = -1;
      ...
      {
         this.var_1300 = new Point(Math.round(Math.random() * 0),Math.round(Math.random() * 0));
      }
      ...
      {
         return this.getFrame(param1) == const_1009;
      }
      
      public function getBitmap(param1:int) : BitmapData
      {
         var _loc2_:int = this.getFrame(param1);
         return this._twinkleImages.getImage(const_1010[_loc2_]);
      }
      
      private function getFrame(param1:int) : int
      {
         var _loc2_:int = param1 - this.var_2267;
         if(_loc2_ < 0)
         {
            return const_1389;
         }
         var _loc3_:int = Math.floor(_loc2_ / const_1388);
         if(_loc3_ >= const_1010.length)
         {
            return const_1009;
         }
         return _loc3_;
