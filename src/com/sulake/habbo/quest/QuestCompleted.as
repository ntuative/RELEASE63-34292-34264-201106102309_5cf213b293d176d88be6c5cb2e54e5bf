package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private static const const_1047:int = 2000;
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         if(this._window == null)
         {
            ...
            this.var_605 = this._questEngine.getTwinkleAnimation(this._window);
         }
         this._window.findChildByName("catalog_link_txt").caption = this._questEngine.localization.getKey("quests.completed.cataloglink." + this.var_121.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_121.activityPointType;
         this._questEngine.localization.registerParameter(_loc2_,"amount",this.var_121.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this._questEngine.localization.getKey(_loc2_,_loc2_);
         this._window.visible = false;
         this._window.findChildByName("congrats_txt").caption = this._questEngine.localization.getKey(!!this.var_121.lastQuestInCampaign ? "quests.completed.campaign.caption" : "quests.completed.quest.caption");
         this._window.findChildByName("more_quests_button").visible = this.var_121.lastQuestInCampaign;
         this._window.findChildByName("campaign_reward_icon").visible = this.var_121.lastQuestInCampaign;
         this._window.findChildByName("catalog_link_region").visible = !this.var_121.lastQuestInCampaign;
         this._window.findChildByName("next_quest_button").visible = !this.var_121.lastQuestInCampaign;
         this._window.findChildByName("reward_icon").visible = !this.var_121.lastQuestInCampaign;
         this._window.findChildByName("campaign_reward_icon").visible = this.var_121.lastQuestInCampaign;
         this._window.findChildByName("campaign_pic_bitmap").visible = this.var_121.lastQuestInCampaign;
         this.setWindowTitle(!!this.var_121.lastQuestInCampaign ? "quests.completed.campaign.title" : "quests.completed.quest.title");
         this.setDesc(this.var_121.getQuestLocalizationKey() + ".completed");
         this._questEngine.setupCampaignImage(this._window,param1,this.var_121.lastQuestInCampaign);
      }
      
      private function setWindowTitle(param1:String) : void
      {
         this._questEngine.localization.registerParameter(param1,"category",this._questEngine.getCampaignName(this.var_121));
      ...
      public function update(param1:uint) : void
      {
         if(this.var_1142 > 0)
         {
            ...
            if(this.var_1142 < 1)
            {
               ...
               if(this.var_121.lastQuestInCampaign)
               {
                  this.var_605.restart();
               }
               else
               {
                  this.var_605.stop();
