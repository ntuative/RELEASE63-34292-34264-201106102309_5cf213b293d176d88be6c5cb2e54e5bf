package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_510 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_510 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2233 = _loc3_.localizationId;
         this.var_415 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_510,this);
               if(_loc5_)
               {
                  this.var_415 = _loc5_.colors;
         ...
         this._offers.add(this.var_510,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_745,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_415)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_582 = param1.index;
         if(this.var_582 < 0 || this.var_582 > this.var_415.length)
         {
            this.var_582 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.method_2 == this.var_2233)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_510 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_228.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_510,_loc5_),this.getRaceLocalizationKey(this.var_510,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_228 == null || this.var_897 >= this.var_228.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_582 >= 0 && this.var_582 < this.var_415.length)
         {
            _loc7_ = this.var_415[this.var_582] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_228[this.var_897] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_510,_loc3_,_loc2_,AvatarScaleType.const_60,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_322,AvatarAction.const_924);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_582 >= this.var_415.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_415[this.var_582] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_228[this.var_897] as int;
