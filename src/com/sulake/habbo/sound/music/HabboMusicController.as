package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      private static const const_1610:int = HabboMusicPrioritiesEnum.const_123;
      ...
      private var var_569:IPlayListController = null;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_751.push(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_751.push(new UserSongDisksInventoryMessageEvent(this.onSongDiskInventoryMessage));
         for each(_loc5_ in this.var_751)
         {
            this._connection.addMessageEvent(_loc5_);
         }
         this.var_88.addEventListener(RoomEngineSoundMachineEvent.const_786,this.onJukeboxInit);
         this.var_88.addEventListener(RoomEngineSoundMachineEvent.const_289,this.onJukeboxDispose);
         this.var_88.addEventListener(RoomEngineSoundMachineEvent.const_927,this.onSoundMachineInit);
         this.var_88.addEventListener(RoomEngineSoundMachineEvent.const_943,this.onSoundMachineDispose);
         this.var_1299 = new Timer(1000);
         this.var_1299.start();
         this.var_1299.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
      ...
      protected function onSongFinishedPlayingEvent(param1:SoundCompleteEvent) : void
      {
         ...
         {
            if(this.getTopRequestPriority() == this._priorityPlaying && this.getSongRequestCountAtPriority(this._priorityPlaying) == this.var_2021)
            {
               this.resetSongStartRequest(this._priorityPlaying);
            }
            _loc2_ = this._priorityPlaying;
            this.playSongWithHighestPriority();
            if(_loc2_ >= HabboMusicPrioritiesEnum.const_395)
            {
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(this._connection)
            {
               for each(_loc1_ in this.var_751)
               {
            ...
            if(this.var_293)
            {
               ...
               {
                  _loc3_ = this.var_293.getWithIndex(_loc2_) as SongDataEntry;
            ...
            if(this.var_88)
            {
               this.var_88.removeEventListener(RoomEngineSoundMachineEvent.const_786,this.onJukeboxInit);
               this.var_88.removeEventListener(RoomEngineSoundMachineEvent.const_289,this.onJukeboxDispose);
               this.var_88.removeEventListener(RoomEngineSoundMachineEvent.const_927,this.onSoundMachineInit);
               this.var_88.removeEventListener(RoomEngineSoundMachineEvent.const_943,this.onSoundMachineDispose);
            }
      ...
      private function getSongDataEntry(param1:int) : SongDataEntry
      {
         ...
         {
            _loc2_ = this.var_293.getValue(param1) as SongDataEntry;
         }
         return _loc2_;
      }
      ...
      {
         return this.var_399.length;
      }
      
      public function getSongDiskInventoryDiskId(param1:int) : int
      {
         if(param1 >= 0 && param1 < this.var_399.length)
         {
            return this.var_399.getKey(param1);
         }
         return -1;
      }
      ...
      public function getSongDiskInventorySongId(param1:int) : int
      {
         if(param1 >= 0 && param1 < this.var_399.length)
         {
            return this.var_399.getWithIndex(param1);
         }
         return -1;
      }
      ...
      public function samplesUnloaded(param1:Array) : void
      {
         ...
         while(_loc2_ < this.var_293.length)
         {
            ...
            if(_loc3_.id != this.var_1598 && _loc4_ != null && _loc4_.ready)
            {
      ...
      public function get samplesIdsInUse() : Array
      {
         ...
         while(_loc2_ < this.var_333.length)
         {
            if(this.var_333[_loc2_] != null)
            {
               ...
               _loc4_ = this.var_293.getValue(_loc3_.songId);
               if(_loc4_ != null)
               {
      ...
      private function sendNextSongRequestMessage(param1:TimerEvent) : void
      {
         ...
         this._connection.send(new GetSongInfoMessageComposer(this.var_858));
         Logger.log("Requested song info\'s : " + this.var_858);
         this.var_858 = new Array();
      }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc5_ < _loc4_.length)
         {
            ...
            if(_loc7_)
            {
               ...
               this.var_293.add(_loc6_.id,_loc10_);
      ...
      private function playSongObject(param1:int, param2:int) : Boolean
      {
         ...
         this.var_2021 = this.getSongRequestCountAtPriority(param1);
         this.var_1598 = param2;
         if(this._priorityPlaying <= const_1610)
         {
            this.notifySongPlaying(_loc4_);
         }
         if(param1 > HabboMusicPrioritiesEnum.const_123)
         {
      ...
      private function notifySongPlaying(param1:SongDataEntry) : void
      {
         ...
         if(param1.length >= 8000 && (this.var_2788 != param1.id || _loc3_ > this.var_2789 + 8000))
         {
            this._soundManager.notifyPlayedSong(param1.name,param1.creator);
            this.var_2788 = param1.id;
            this.var_2789 = _loc3_;
         }
      }
      ...
      private function onSongDiskInventoryMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDiskCount)
         {
            ...
            if(this.var_293.getValue(_loc6_) == null)
            {
      ...
      private function onSoundMachineInit(param1:Event) : void
      {
         ...
         this.var_569 = new SoundMachinePlayListController(this._soundManager,this,this._events,this.var_88,this._connection) as IPlayListController;
      }
      ...
      {
         this.disposeRoomPlaylist();
         this.var_569 = new JukeboxPlayListController(this._soundManager,this,this._events,this._connection) as IPlayListController;
