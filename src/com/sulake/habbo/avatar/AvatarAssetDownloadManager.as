package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_430:AssetLoaderStruct;
      
      private var var_929:Timer;
      ...
      private const const_2267:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_430 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_929 = new Timer(this.const_2267,1);
         this.var_929.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_430)
         {
            this.var_430.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_430.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_430)
         {
            this.var_430.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_430.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_927 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1145);
         ...
         else
         {
            ...
            if(this.var_1145.indexOf("?") > 0)
            {
               _loc2_ = this.var_1145 + "&retry=" + this.var_927;
            }
            else
            {
               _loc2_ = this.var_1145 + "?retry=" + this.var_927;
            ...
            this.var_430 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2289,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1147[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1147[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1720)
         {
            this.var_1146.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_513[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_513[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_928)
         {
            _loc7_ = true;
            _loc4_ = this.var_928[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_513[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_928[_loc3_];
         }
         while(_loc6_ < this.var_744.length)
         {
            _loc5_ = this.var_744[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_744.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_929.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_743.length > 0 && this.var_744.length < this.const_1979)
         {
            _loc1_ = this.var_743[0];
            _loc1_.startDownloading();
            this.var_744.push(this.var_743.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_743.indexOf(param1) == -1 && this.var_744.indexOf(param1) == -1)
         {
