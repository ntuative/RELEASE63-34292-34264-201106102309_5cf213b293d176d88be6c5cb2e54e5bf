package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_810:IFigurePartSet;
      ...
      private var var_461:IAvatarRenderManager;
      
      private var var_1123:Boolean;
      
      private var var_1902:BitmapData;
      
      private const const_2281:int = 13421772;
      
      private const const_2282:int = 16777215;
      
      private const const_377:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1901 = Math.max(this.var_1901,_loc6_.colorLayerIndex);
            }
         }
         this.var_461 = this.var_76.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_729 != null && !this.var_1900)
            {
               _loc3_ = this.var_729;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_810)
            {
               switch(this.var_810.clubLevel)
               {
                  case HabboClubLevelEnum.const_55:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_38:
                     _loc2_.style = HabboIconType.const_946;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_958;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1520.color = this.const_2281;
         }
         else
         {
            this.var_1520.color = this.const_2282;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_461.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_461.isFigureReady(_loc1_))
         {
            this.var_461.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_377[_loc6_] + "_" + FigureData.const_155;
               _loc4_ = this.var_461.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_377.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_377[_loc6_] + "_" + FigureData.const_155;
                  _loc4_ = this.var_461.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_663 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1902)
               {
                  ...
                  this.var_1902 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1900 && _loc2_.colorLayerIndex > 0)
               {
