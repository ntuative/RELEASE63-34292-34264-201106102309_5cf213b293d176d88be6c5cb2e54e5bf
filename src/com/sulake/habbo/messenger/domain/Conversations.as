package com.sulake.habbo.messenger.domain
{
   ...
   public class Conversations
   {
      ...
      private static var var_1244:int = 1;
      ...
      public function setSelected(param1:Conversation) : void
      {
         ...
         var _loc2_:int = this.var_148.indexOf(param1);
      ...
      public function closeConversation() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc2_ = this.var_148.indexOf(_loc1_);
            Util.remove(this.var_148,_loc1_);
            this._closedConversations.push(_loc1_);
            _loc1_.setSelected(false);
            while(_loc2_ >= 0)
            {
               if(this.var_148[_loc2_] != null)
               {
                  this.var_148[_loc2_].setSelected(true);
                  break;
               }
               _loc2_--;
            }
      ...
      {
         this.addConversation(var_1244++);
      }
      
      public function addConversation(param1:int) : Conversation
      {
         var _loc2_:Boolean = this._closedConversations.length == 0 && this.var_148.length == 0;
         var _loc3_:Conversation = this.addConversationInt(param1);
         if(_loc3_ == null)
         {
            return null;
         }
         if(this.var_148.length == 1)
         {
            this.var_148[0].setSelected(true);
         }
         if(_loc2_)
         {
            _loc3_.addMessage(new Message(Message.const_727,0,this.var_459.getText("messenger.moderationinfo"),""));
         }
         return _loc3_;
      }
      ...
      {
         var _loc2_:* = null;
         for each(_loc2_ in this.var_148)
         {
            if(_loc2_.id == param1)
            {
               return _loc2_;
      ...
      {
         var _loc2_:int = this.var_148.length;
         ...
         if(_loc2_ != this.var_148.length || _loc4_ != _loc3_.newMessageArrived)
         {
      ...
      {
         this._startIndex = Math.min(this._startIndex,this.var_148.length - this.var_459.getTabCount());
         this._startIndex = Math.max(0,this._startIndex);
      }
      
      private function getOnlineInfoMsg(param1:Boolean) : Message
      {
         return new Message(Message.const_883,0,this.getOnlineText(param1),Util.getFormattedNow());
      }
