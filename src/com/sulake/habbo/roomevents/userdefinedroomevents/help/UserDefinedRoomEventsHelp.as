package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_2138 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1362 = this.var_2138.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1362)
         {
            this.var_1362.procedure = this.onButtonClick;
         }
         this.var_1363 = this.var_2138.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1363)
         {
            this.var_1363.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_301 >= this.var_70.length - 1)
         {
            this.var_1363.disable();
         }
         else
         {
            this.var_1363.enable();
         }
         if(this.var_301 <= 0)
         {
            this.var_1362.disable();
         }
         else
         {
            this.var_1362.enable();
         }
      }
      ...
      {
         ++this.var_301;
         this.var_301 = Math.min(this.var_301,this.var_70.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_301;
         this.var_301 = Math.max(this.var_301,0);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_70[this.var_301][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_70[this.var_301][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_70[this.var_301][1]).value;
