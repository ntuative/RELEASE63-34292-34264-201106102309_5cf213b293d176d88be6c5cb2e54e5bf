package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_644:String = "InitConnection";
       
      
      private var var_176:HabboCommunicationDemo;
      ...
      {
         this.var_176.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_176 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_176.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_155,HabboWindowStyle.const_46,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_176.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2014,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_176.habboConfiguration)
            {
               if(this.var_176.habboConfiguration.getKey("use.sso","false") == "UserTypingMessageEvent")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_176.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_176.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2014,"/");
         ...
         {
            this.method_12(this.useExistingSession);
         }
         else
         {
            dispatchEvent(new Event(const_644));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_176.sendTryLogin(this.name,this.password,param2.id);
      }
      
      private function method_12(param1:Boolean) : void
      {
         if(this.var_154 != null)
         {
            this.var_154.removeEventListener(SSOTicket.const_422,this.onParsedTicketSuccess);
            this.var_154.removeEventListener(SSOTicket.const_328,this.onParsedTicketFailure);
            this.var_154 = null;
         }
         var _loc2_:String = "";
         if(this.var_176.habboConfiguration != null)
         {
            _loc2_ = this.var_176.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_176.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_154 = new SSOTicket(this.var_176.assets,_loc3_,_loc2_,param1);
         this.var_154.addEventListener(SSOTicket.const_422,this.onParsedTicketSuccess);
         this.var_154.addEventListener(SSOTicket.const_328,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_154.ticket);
         Logger.log("Has Facebook: " + this.var_154.isFacebookUser);
         if(this.var_154.isFacebookUser)
         {
            this.var_176.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_176.flashClientUrl = this.var_154.flashClientUrl;
         this.var_176.ssoTicket = this.var_154.ticket;
         this.var_154.removeEventListener(SSOTicket.const_422,this.onParsedTicketSuccess);
         this.var_154.removeEventListener(SSOTicket.const_328,this.onParsedTicketFailure);
         this.var_154 = null;
         dispatchEvent(new Event(const_644));
