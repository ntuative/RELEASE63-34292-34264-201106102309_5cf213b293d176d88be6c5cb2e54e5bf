package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_328:String = "failure";
      ...
      private var var_1529:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1528 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1529.init();
            this.var_1529.addEventListener(HabboWeb.const_266,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1528;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1528);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1528);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1529.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1917 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1933 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2620 = Boolean(facebookData[1]);
               }
               if(this.var_1917.length > 0)
               {
