package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         {
            if(this.var_303[_loc3_].url == param1.url)
            {
               return this.var_303[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
      ...
      protected function removeLibraryLoader(param1:LibraryLoader) : void
      {
         ...
         {
            if(this.var_303[_loc2_] == param1)
            {
               this.var_303.splice(_loc2_,1);
               break;
            }
            _loc2_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_730);
                  }
                  k++;
               }
            }
            ...
            if(componentClass == null)
            {
               this.error("Invalid component class " + componentString + "!",true,Core.const_730);
               return null;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     this.error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_730);
                  }
               }
      ...
      public final function attachComponent(param1:Component, param2:Array) : void
      {
         ...
         if(param1.locked)
         {
            param1.events.addEventListener(const_457,this.lockEventHandler);
         }
      ...
      public final function detachComponent(param1:Component) : void
      {
         ...
         while(_loc5_ < this._components.length)
         {
            if(this._components[_loc5_] == param1)
            {
               ...
               param1.events.removeEventListener(const_457,this.lockEventHandler);
               return;
            }
            _loc5_++;
         }
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               this.error("Interface " + param1 + " still unavailabe!",true,Core.const_1891);
            }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(this.var_303 != null)
            {
               _loc1_ = uint(this.var_303.length);
               while(_loc1_-- > 0)
               {
                  _loc2_ = this.var_303[0];
                  this.removeLibraryLoader(_loc2_);
                  _loc2_.dispose();
               }
               this.var_303 = null;
      ...
      private function lockEventHandler(param1:LockEvent) : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:* = null;
         if(param1.type == const_457)
         {
            _loc2_ = param1.unknown as Component;
            _loc2_.events.removeEventListener(const_457,this.lockEventHandler);
            if(!disposed)
            {
               _loc3_ = new Array();
