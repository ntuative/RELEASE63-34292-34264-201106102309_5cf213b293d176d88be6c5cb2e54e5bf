package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1485:String = "asset-libraries";
      
      private static const const_1486:String = "library";
      
      private static const const_1490:String = "service-libraries";
      
      private static const const_1488:String = "library";
      
      private static const const_1484:String = "component-libraries";
      
      private static const const_1487:String = "library";
      
      private static const const_1491:String = "error_data";
      
      private static const const_1483:String = "error_cat";
      
      private static const const_1489:String = "error_desc";
       
      
      private var var_374:LibraryLoaderQueue;
      
      private var var_798:IEventDispatcher;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_421.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1366:
               debug("Core; using simple frame update handler");
               this.var_373 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_757:
               debug("Core; using complex frame update handler");
               this.var_373 = this.complexFrameUpdateHandler;
               break;
            case Core.const_806:
               debug("Core; using profiler frame update handler");
               this.var_373 = this.profilerFrameUpdateHandler;
               this.var_217 = new Profiler(this);
               attachComponent(this.var_217,[new IIDProfiler()]);
               var_421.addChild(new ProfilerViewer(this.var_217));
               break;
            case Core.const_655:
               debug("Core; using experimental frame update handler");
               this.var_373 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_373 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_545 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(var_421)
            {
               var_421.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_421 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1491,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1483,String(param3));
         ErrorReportStorage.setParameter(const_1489,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_798 = loadingEventDelegate;
         if(this.var_374 == null)
         {
         ...
         try
         {
            node = config.child(const_1485)[0];
            if(node != null)
            {
               list = node.child(const_1486);
         ...
         try
         {
            node = config.child(const_1490)[0];
            if(node != null)
            {
               list = node.child(const_1488);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1484)[0];
            if(node != null)
            {
               list = node.child(const_1487);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1299);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_798)
         {
            this.var_798.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_798)
         {
            ...
            this.var_798.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_798)
                  {
                     this.var_798.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_545 - this.var_374.length,this.var_545,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_217 ? 0 : this.var_1499 & 0;
         if(_loc3_ == Core.const_655)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_373(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_421.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_106)
         {
            t = getTimer();
            ...
            {
               if(this.var_655[priority] < priority)
               {
                  ++this.var_655[priority];
                  skip = true;
               }
            }
            if(!skip)
