package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1995:uint = 1;
      
      protected static const const_1993:uint = 2;
      
      protected static const const_1994:uint = 4;
      
      protected static const const_1996:uint = 3;
      
      protected static const name_4:uint = 7;
      
      public static const const_1813:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1175:Boolean = false;
      
      public static const const_1979:int = 2;
      
      private static var var_1083:Array = new Array();
      
      private static var var_880:Array = new Array();
      ...
      protected var var_261:URLRequest;
      ...
      protected var var_3005:String = "";
      
      protected var var_2226:int;
      
      protected var var_1409:int;
      
      protected var var_1408:int;
      
      protected var var_3063:uint = 0;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_45.load(param1.var_261,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1175)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_880[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_880.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_880.length < const_1979 && false)
            {
               _loc1_ = var_1083.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_880.push(_loc1_);
                  _loc1_.var_45.load(_loc1_.var_261,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_261 ? this.var_261.url : null;
      }
      ...
      {
         return !!this.var_1131 ? uint(this.var_1409 - this.var_2226) : uint(getTimer() - this.var_2226);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_425,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_45.close();
            }
      ...
      {
         this.var_261 = param1;
         this._name = parseNameFromUrl(this.var_261.url);
         this.var_1131 = false;
      ...
      {
         if(this.var_1132 && !_disposed)
         {
            this.var_1132 = false;
            if(!this.var_1131 && this.var_261)
            {
               this.var_1409 = -1;
      ...
      {
         if(!this.var_1131 && !_disposed && this.var_1408 > 0)
         {
            try
            {
               this.var_45.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_261,const_1813 - this.var_1408);
            if(this.var_3004)
            {
               this.var_261.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_261.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_3004 = false;
            }
            --this.var_1408;
            this.var_506 = 0;
            this.var_45.load(this.var_261,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_425 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_425,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_425,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_425))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_506 = LibraryLoader.name_4;
         ...
         if(this.var_506 == LibraryLoader.const_1996)
         {
         ...
         if(this.var_506 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_425,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         if(this.var_652 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_261.url + "!");
            }
            return false;
         }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_425,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_425,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
