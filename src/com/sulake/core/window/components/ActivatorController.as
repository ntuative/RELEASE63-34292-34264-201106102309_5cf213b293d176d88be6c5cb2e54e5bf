package com.sulake.core.window.components
{
   ...
   public class ActivatorController extends ContainerController
   {
      ...
      {
         if(param2.type == WindowEvent.const_333)
         {
      ...
      public function setActiveChild(param1:IWindow) : IWindow
      {
         ...
         var _loc2_:IWindow = this.var_913;
         if(this.var_913 != param1)
         {
            if(this.var_913 != null)
            {
               if(!this.var_913.disposed)
               {
                  this.var_913.deactivate();
               }
            }
            this.var_913 = param1;
